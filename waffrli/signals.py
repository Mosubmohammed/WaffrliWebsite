...
def clear_cache_on_product_save(sender, instance, **kwargs):
    ...
    cache.delete(f'home_images_{current_day}')
    
    cache.delete('home_page_header')              
    cache.delete('featured_deals_slider')         
    cache.delete('category_separator')            
    cache.delete('hot_deals_slider')              
    cache.delete('popular_products_slider')       
    cache.delete('category_products_section')     
    
    # Clear paginated product grids (clear multiple pages)
    for page_num in range(1, 6):  # Clear first 5 pages
        cache.delete(f'all_products_grid:{page_num}')
